...
class Absen_setoran_set extends MX_Controller {

    public function __construct() {
        parent::__construct();
        $this->load->model('student/Student_model');
        $this->load->model('absen_setoran/Absen_setoran_model');
    ...
    public function index() {
        ...
        $data = array(
            ...
            'f' => $f,
            'top_absent' => $this->Absen_setoran_model->get_top_absent($active_period['period_id']), 
        ...
        if (!empty($f['n']) && !empty($f['r'])) {
            ...
            if (!empty($data['santri_selected'])) {
                ...
                $data['absen'] = $this->Absen_setoran_model->get_by_student($student_id, $period_id);
                $data['total_absen'] = $this->Absen_setoran_model->get_total_absen($student_id, $period_id);
            }
        }

        $data['title'] = 'Pelanggaran Absen Setoran';
        $data['main'] = 'absen_setoran/setoran_filter';
    ...
    public function add() {
        if ($this->input->post()) {
            $data = array(
                ...
                'jumlah_tidak_setoran' => $this->input->post('jumlah_tidak_setoran'), // Ambil input manual
                'keterangan' => $this->input->post('keterangan')
            );
    
            // Validasi dasar
            if (empty($data['jumlah_tidak_setoran']) || $data['jumlah_tidak_setoran'] < 1) {
            ...
            // Proses penyimpanan
            if ($this->Absen_setoran_model->add($data)) {
                $this->session->set_flashdata('success', 'Data berhasil disimpan');
            } else {
                $this->session->set_flashdata('error', 'Gagal menyimpan data');
            }
            
            redirect('manage/absen_setoran?n='.$data['period_id'].'&r='.$this->input->post('student_nis'));
        }
    }
        ...
        public function send_whatsapp($id) {
            ...
            // Ambil data absen setoran
            $absen = $this->Absen_setoran_model->get_by_id($id);
            
            if (!$absen) {
                $this->session->set_flashdata('error', 'Data absen tidak ditemukan');
                redirect('manage/absen_setoran');
            }
            ...
            $message = "Assalamu'alaikum Warahmatullahi Wabarakatuh\n\n"
                ...
                . "Dengan hormat, kami ingin menyampaikan informasi mengenai ketidakhadiran Ananda dalam kegiatan setoran:\n\n"
                . "*Periode :*\n"
                . "Mulai: {$tgl_mulai}\n"
                . "Selesai: {$tgl_selesai}\n"
                . "_Atau Dalam 2 Minggu Terakhir Ini_\n\n"
                . "*Detail Ketidakhadiran:*\n"
                . "Jumlah Tidak Hadir Setoran : {$absen['jumlah_tidak_setoran']} Kali\n"
                . "Keterangan: {$absen['keterangan']}\n\n"
                . "Kami mengharapkan peran serta Bapak/Ibu untuk memberikan motivasi dan pengawasan kepada Ananda agar dapat mengikuti kegiatan setoran dengan lebih baik.\n\n"
            ...
            curl_close($ch);
            redirect('manage/absen_setoran?n='.$this->input->get('n').'&r='.$this->input->get('r'));
        }
    ...
    public function delete($id) {
        ...
        if ($this->Absen_setoran_model->delete($id)) {
            $this->session->set_flashdata('success', 'Data absen dihapus.');
        } else {
            $this->session->set_flashdata('error', 'Gagal menghapus data.');
        }
        redirect('manage/absen_setoran?n='.$period_id.'&r='.$student_nis);
    }
}